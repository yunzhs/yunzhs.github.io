...
主要问题一个,署了授权服务器后,发现在cloud环境下,资源服务器向授权服务器请求得来的用户信息无法认证scope等权限,在尝试了网上的多种配置无效后,我抱着爆炸的心态查看源码,意外的发现可能是源码错误,而且是相当明显的错误,它验证了一个它自己赋值为null的值,这就让我只能修改源码了,没办法的办法.
